...
intervals_average = (
    {}
)  # dict that we fill with and array per sensors that needs an average value. Number of values in array is based on "interval" var in config file
intervals_counter = (
    {}
)  # counter to use per device in dict so we know when to stop. :-) global since it iterates and this was the best I could come up with.
...
def stop_thread(thread_id):
    ...
    if thread_id in str(dThreads):
        logging.warning(
            "{}: Thread {} found in thread list: {} , checking if running or started...".format(
                get_function_name(), dThreads[thread_id], dThreads
            )
        )
        ...
        if thread.is_running():
            logging.warning(
                "{}: Thread {} IS found active in running threads: {} , proceeding to stop {}.".format(
                    get_function_name(), thread_id, dThreads, dThreads[thread_id]
                )
            )
            logging.warning(
                '{}: Stopping thread "{}"'.format(get_function_name(), thread_id)
            )
            thread.stop_me()
            logging.warning(
                "{}: thread.stop_me finished. Now running threads and status: {} .".format(
                    get_function_name(), dThreads
                )
            )
            thread.join(
                10
            )  # implemented a timeout of 10 since the join here is blocking and halts the complete script. this allows the main function to continue, but is an ERROR since a thread is not joining. Most likely an function that hangs (function needs to end before a join is succesfull)!
            thread.stopped()
            logging.warning(
                '{}: Stopped thread "{}"'.format(get_function_name(), thread_id)
            )
            del dThreads[thread_id]
            logging.warning(
                '{}: Remaining running threads are "{}".'.format(
                    get_function_name(), dThreads
                )
            )
        else:
            logging.warning(
                "{}: Thread {} not running or started.".format(
                    get_function_name(), dThreads[thread_id]
                )
            )
    else:
        logging.warning(
            "{}: Thread {} not found in global thread var: {}.".format(
                get_function_name(), thread_id, dThreads
            )
        )
...
def every(delay, task):
    ...
    while True:
        ...
        try:
            ...
            logging.warning(
                "{}: Problem while executing repetitive task.".format(
                    get_function_name()
                )
            )
...
def on_mqtt_message(unused1, unused2, msg):
    del unused1, unused2
    if "set" in msg.topic:
        mqtt_msg = str(msg.payload.decode("utf-8", "ignore"))
        logging.debug(
            '{}: Message "{}" on input.'.format(get_function_name(), mqtt_msg)
        )
        if mqtt_msg.startswith("{"):
            try:
                # noinspection PyTypeChecker
                mqtt_msg_json = json.loads(
                    mqtt_msg, object_pairs_hook=OrderedDict
                )  # need the orderedDict here otherwise the order of the mQTT message is changed, that will bnreak the return message and than the device won't turn on in HASSIO
            except ValueError as e:
                logging.error(
                    '{}: Message "{}" not a valid JSON - message not processed, error is "{}".'.format(
                        get_function_name(), mqtt_msg, e
                    )
                )
            else:
                logging.debug(
                    '{}: Message "{}" is a valid JSON, processing json in handle_json.'.format(
                        get_function_name(), mqtt_msg_json
                    )
                )
                handle_json(msg.topic, mqtt_msg_json)
        else:
            logging.debug(
                '{}: Message "{}" not JSON format, processing other format.'.format(
                    get_function_name(), mqtt_msg
                )
            )
            handle_other(msg.topic, mqtt_msg)


# Main function to handle incomming MQTT messages, check content en start the correct function to handle the request.
...
def handle_json(ms_topic, message):
    ...
    try:
        ...
        # Transition, optional. You can fade anolog outputs slowly. Transition is the amount of seconds you want to
        # fade to take (seconds always applied to 0-100%, so 0-25% = 25% of seconds)
        transition_presence = "transition" in message
        if transition_presence:
            transition_value = message["transition"]
        # Brightness, if you switch lights with 0-10 volt we translate the input value (0-255) to 0-10 and consider
        # this brightness
        brightness_presence = "brightness" in message
        if brightness_presence:
            brightness_value = message["brightness"]
        # Repeat, if present this will trigger an on - off action x amout of times. I use this to trigger a relay
        # multiple times to let a bel ring x amount of times.
        repeat_presence = "repeat" in message
        if repeat_presence:
            repeat_value = message["repeat"]
    ...
    if (
        ...
        if transition_presence:
            if brightness_presence:
                ...
                logging.warning(
                    '{}: started thread "{}" for "transition" of dev "{}" circuit "{}".'.format(
                        get_function_name(),
                        dThreads[thread_id],
                        circuit_value,
                        state_value,
            ...
            stop_thread(thread_id)
            set_brightness(
                brightness_value, circuit_value, ms_topic, message
            )  # not in thread as this is not blocking
        elif effect_presence:
            logging.error(
                '{}: Processing "effect", but not yet implemented, aborting. Message data is "{}"'.format(
                    get_function_name(), message
                )
            )
            ...
            dThreads[thread_id].start()
            logging.debug(
                '{}: started thread "{}" for "duration" of dev "{}" circuit "{}".'.format(
                    get_function_name(), dThreads[thread_id], circuit_value, state_value
                )
            )
        elif repeat_presence:
            logging.debug(
                '{}: starting "repeat" message handling for dev "{}" circuit "{}" for {} time'.format(
            ...
            dThreads[thread_id].start()
            logging.debug(
                '{}: started thread "{}" for "repeat" of dev "{}" circuit "{}".'.format(
                    get_function_name(), dThreads[thread_id], circuit_value, state_value
                )
            )
            ...
            stop_thread(thread_id)
            set_state(
                dev_value, circuit_value, state_value, ms_topic, message
            )  # not in thread, not blocking
        else:
...
def ws_sanity_check(message):
    ...
    if type(mesdata) is dict:
        ...
        logging.debug(
            "DICT message without converting (will be processed): {}".format(message)
        )
    else:
        for (
            message_dev
            ...
            if type(message_dev) is dict:
                ...
                logging.debug(
                    "Ignoring received data, it is not a dict: {}".format(message_dev)
                )
        ...
        if (
            ...
            # Implemented device types per 2020 to filter counter for pulse based counter devices like water meters. Just count counter on NO devices that turn on. Since WebDav does not send a trigger for every update we calculate the delta betwee this and the previous update.
                ...
                if config_dev["device_type"] == "counter":
                    ...
                    logging.error(
                        '{}: Unknown device type "{}", breaking.'.format(
                ...
                logging.debug(
                    "{}: Loop with delay with message: {}".format(
                        get_function_name(), message_dev
                    )
                )
                if tijd >= (
                    ...
                    if message_dev["value"] == 1:
                        if config_dev["unipi_value"] == 1:
                            logging.debug(
                                "{}: received status 1 is actual status: {}".format(
                                    get_function_name(), message_dev
                                )
                            )  # nothing to do, since there is not status change. First in condition to easy load ;-)
                            ...
                            if handle_local_presence:
                                handle_local_switch_on_or_toggle(
                                    message_dev, config_dev
                                )
                            ...
                            logging.error(
                                "{}: Unhandled Exception 1, config: {}, status: {}, normal_config: {}, {}, {}".format(
                                    get_function_name(),
                                    config_dev["unipi_value"],
                                    message_dev["value"],
                                    config_dev["device_normal"],
                                    message_dev["circuit"],
                        ...
                        if config_dev["unipi_value"] == 0:
                            logging.debug(
                                "{}: received status 0 is actual status: {}".format(
                                    get_function_name(), message_dev
                                )
                            )  # nothing to do, since there is not status change. First in condition to easy load ;-)
                ...
                logging.debug(
                    "{}: Loop without delay with message: {}".format(
                        get_function_name(), message_dev
                    )
                )
                if message_dev["value"] == 1:
                    if config_dev["device_normal"] == "no":
                        ...
                        logging.debug(
                            "{}: ERROR 1, config: {}, normal_config: {}, {}, {}".format(
                                get_function_name(),
                                message_dev["value"],
                                config_dev["device_normal"],
                                message_dev["circuit"],
                    ...
                    if config_dev["device_normal"] == "no":
                        ...
                        logging.debug(
                            "{}: ERROR 2, config: {}, normal_config: {}, {}, {}".format(
                                get_function_name(),
                                message_dev["value"],
                                config_dev["device_normal"],
                                message_dev["circuit"],
            ...
            if int_presence:
                ...
                if cntr <= config_dev["interval"]:
                    intervals_average[config_dev["dev"] + config_dev["circuit"]][
                        cntr
                    ] = float(round(message_dev["value"], 3))
                    ...
                    logging.debug(
                        "PING Received WebSocket data and collected 30 samples of lux data : {}".format(
                            message_dev
                        )
                    )  # we're loosing websocket connection, debug
...
def dev_switch_on(mqtt_topic):
    ...
    logging.info(
        '{}: Set ON for MQTT topic: "{}".'.format(get_function_name(), mqtt_topic)
    )
...
def dev_switch_off(mqtt_topic):
    ...
    logging.info(
        '{}: Set OFF for MQTT topic: "{}".'.format(get_function_name(), mqtt_topic)
    )
...
def mqtt_set_lux(mqtt_topic, lux):
    try:
        ...
        logging.info(
            '{}: Set LUX: {} for MQTT topic: "{}" .'.format(
                get_function_name(), lux, mqtt_topic
            )
        )
    except:
        logging.error(
            '{}: An error has occurred sending "{}" C for MQTT topic: "{}" .'.format(
                get_function_name(), lux, mqtt_topic
            )
        )
...
def mqtt_set_temp(mqtt_topic, temp):
    try:
        send_msg = {"temperature": temp}
        mqttc.publish(mqtt_topic, payload=json.dumps(send_msg), qos=1, retain=False)
        logging.info(
            '{}: Set temperature: {} C for MQTT topic: "{}" .'.format(
                get_function_name(), temp, mqtt_topic
            )
        )
    except:
        logging.error(
...
def mqtt_set_humi(mqtt_topic, humi):
    try:
        send_msg = {"humidity": humi}
        mqttc.publish(mqtt_topic, payload=json.dumps(send_msg), qos=1, retain=False)
        logging.info(
            '{}: Set humidity: {} for MQTT topic: "{}" .'.format(
                get_function_name(), humi, mqtt_topic
            )
        )
    except:
        logging.error(
                    ...
                    if config_dev["dev"] == "temp":
                        if cntr <= config_dev["interval"]:
                            if message_dev["typ"] == "DS18B20":
                                ...
                                logging.error(
                                    "{}: Unknown Device sensor type {} in config".format(
                                        get_function_name(), message_dev["typ"]
                                    )
                                )
                        else:
...
def set_repeat(dev, circuit, repeat, topic, message):
    ...
    while repeat > ctr and thread.is_running():
        ...
        if thread.is_stopping():
            ...
            unipy.set_off(
                dev, circuit
            )  # extra off since we need to make sure my bel is off, or it will burn out. :-(
        else:
            if int(stat_code_off) == 200 or int(stat_code_on) == 200:
                ...
                message.pop(
                    "repeat"
                )  # remove repeat from final mqtt ack with orderd dict action
                message.update(
                    {"state": "off"}
                )  # replace state "pulse" with "off" with orderd dict action
                mqtt_ack(topic, message)
                logging.info(
                    "    {}: Successful ran function on dev {} circuit {} for {} times.".format(
                        get_function_name(), dev, circuit, repeat
                    )
                )
            else:
                logging.error(
...
def set_state(dev, circuit, state, topic, message):
    ...
    if dev == "analogoutput" and state == "on":
        ...
        if int(stat_code) == 200:
            mqtt_ack(topic, message)
            logging.info(
                "    {}: Successful ran function on device {} circuit {} to state {}.".format(
                    get_function_name(), dev, circuit, state
                )
            )
        else:
            logging.error(
        ...
        logging.error(
            "   {}: Unhandled exception in function.".format(get_function_name())
        )
    del dThreads[thread_id]
    logging.debug("   {}: EOF.".format(get_function_name()))
    ...
    if dev == "analogoutput" and state == "on":
        ...
        if (
            ...
            logging.info(
                '    {}: Set {} for circuit "{}".'.format(
                    get_function_name(), state, circuit
                )
            )
        else:
            logging.error(
                "   {}: error switching device {} on UniPi {}.".format(
                    get_function_name(), circuit, stat_code
                )
            )
        while counter > 0 and thread.is_running():
            ...
            if (
                ...
                logging.info(
                    '    {}: Set {} for circuit "{}".'.format(
                        get_function_name(), rev_state, circuit
                    )
                )
            else:
                logging.error(
                    "   {}: error switching device {} to {} on UniPi {}.".format(
                        get_function_name(), circuit, rev_state, stat_code
                    )
                )
            if thread.is_stopping():
                logging.warning(
...
def set_brightness(desired_brightness, circuit, topic, message):
    logging.debug(
        '   {}: Starting with message "{}".'.format(get_function_name(), message)
    )
    ...
    if stat_code == 200:
        ...
        logging.info(
            '    {}: Set {} for circuit "{}".'.format(
                get_function_name(), brightness_volt, circuit
            )
        )
    else:
...
def transition_brightness(desired_brightness, trans_time, dev, circuit, topic, message):
    logging.debug(
        '   {}: Starting function with message "{}".'.format(
            get_function_name(), message
        )
    )
    global dThreads
    ...
    thread = dThreads[thread_id]
    logging.info(
        "   {}:thread information from global thread var {}".format(
            get_function_name(), dThreads
        )
    )
    trans_step = round(
        float(trans_time) / 100, 3
    )  # determine time per step for 100 steps. Fix for 100 so dimming is always the same speed, independent of from and to levels
    current_level = unipy.get_circuit(
        dev, circuit
    )  # get current circuit level from unipi REST
    desired_level = round(
    ...
    )  # determine number of steps based on from and to level
    ...
    if number_steps != 0:
        if delta_level != number_steps:
            ...
            while int(number_steps) > 0 and thread.is_running():
                ...
                message.update(
                    {"brightness": temp_level}
                )  # replace requested level with actual level in orderd dict action
                mqtt_ack(topic, message)
                number_steps -= 1
                if number_steps > 0:
                    ...
                    logging.info(
                        "   {}: Done setting brightness via WebSocket.".format(
                            get_function_name()
                        )
                    )
                    ...
                    time.sleep(
                        1.5
                    )  # need a sleep here since getting actual value back is slow sometimes, it takes about a second to get the final value.
                    actual_level = unipy.get_circuit(dev, circuit)
                    logging.info(
                        '   {}: Got actual level of "{}" back from function unipy.get_circuit.'.format(
                    ...
                    logging.error(
                        "   {}: Unhandled Condition".format(get_function_name())
                    )
            ...
            logging.error(
                "    {}: delta_level != number_steps.".format(
            ...
            if (
                ...
                if tijd >= (
                    ...
                    if (config_dev["counter_value"] >= config_dev["unipi_value"]) and (
                        ...
                        config_dev["unipi_prev_value_timstamp"] = tijd
                        if counter != delta:
                            mqtt_set_counter(state_topic, counter, delta)
                        else:
                            logging.warning(
                        ...
                        logging.error(
                            "{}: Negative value!.".format(get_function_name())
                        )
                        logging.error(
                            "{}:  - config: {}".format(get_function_name(), config_dev)
                        )
            else:
                logging.error(
                    '{}: Unknown device type "{}", breaking.'.format(
...
def mqtt_set_counter(
    ...
    mqttc.publish(mqtt_topic, payload=json.dumps(send_msg), qos=1, retain=False)
    logging.info(
        '{}: Set counter {} and delta: {} for topic "{}" .'.format(
            get_function_name(), counter, delta, mqtt_topic
        )
    )
...
def mqtt_topic_ack(mqtt_topic, mqtt_message):
    ...
    logging.info(
        '{}: Send MQTT message: "{}" for MQTT topic: "{}" .'.format(
            get_function_name(), mqtt_message, mqtt_topic
        )
    )
...
def mqtt_topic_set(mqtt_topic, mqtt_message):
    ...
    mqttc.publish(
        mqtt_topic, payload=mqtt_message, qos=1, retain=True
    )  # changed retain to true as HASS does a retain true for most messages. Meaning actual state is not maintained to last resort.
    logging.info(
        '{}: Send MQTT message: "{}" for MQTT topic: "{}" .'.format(
            get_function_name(), mqtt_message, mqtt_topic
        )
...
def handle_local_switch_on_or_toggle(message_dev, config_dev):
    logging.debug(
        "{}: Handle Local ON for message: {} and handle_local_config {}.".format(
            get_function_name(), message_dev, config_dev["handle_local"]
        )
    )
    if config_dev["handle_local"]["type"] == "bel":
        ...
        logging.info(
            "{}: Handle Local is ringing the bel {} times".format(
                get_function_name(), config_dev["handle_local"]["rings"]
            )
        )
        state_topic = f"{unipiConfig['inputTopic']}/{config_dev['name']}"
        mqtt_message = "ON"
...
def handle_local_switch_toggle(message_dev, config_dev):
    ...
    if config_dev["handle_local"]["type"] == "dimmer":
        ...
        status, success = unipy.toggle_dimmer(
            "analogoutput",
            config_dev["handle_local"]["output_circuit"],
        ...
        if (
            ...
            if status == 0:
                ...
                logging.info(
                    "{}: Handle Local toggled analogoutput {} to OFF".format(
                        get_function_name(),
                        config_dev["handle_local"]["output_circuit"],
                    )
                )
                ...
                logging.info(
                    "{}: Handle Local toggled analogoutput {} to ON".format(
                        get_function_name(),
                        config_dev["handle_local"]["output_circuit"],
                    )
                )
                ...
                logging.error(
                    '{}: Received error from rest call with code "{}" on analogoutput {}.'.format(
                        get_function_name(),
                        status,
                        config_dev["handle_local"]["output_circuit"],
                    )
                ...
                logging.error(
                    '{}: "status" not 0,1,666 or 667 while running "dimmer loop"."'.format(
                        get_function_name()
                    )
                )
        else:
            logging.error(
                '{}: Tried to toggle analogoutput {} but failed with http return code "{}" .'.format(
                    get_function_name(),
                    config_dev["handle_local"]["output_circuit"],
                    success,
                )
        ...
        logging.debug(
            'Switch Toggle Running function : "{}"'.format(get_function_name())
        )
        status, success = unipy.toggle_switch(
            "output", config_dev["handle_local"]["output_circuit"]
        )
        if success == 200:
            if status == 0:
                ...
                logging.info(
                    "{}: Handle Local toggled output {} to OFF".format(
                        get_function_name(),
                        config_dev["handle_local"]["output_circuit"],
                    )
                )
                ...
                logging.info(
                    "{}: Handle Local toggled output {} to ON".format(
                        get_function_name(),
                        config_dev["handle_local"]["output_circuit"],
                    )
                )
                ...
                logging.error(
                    '{}: Received error from rest call with code "{}" on output {}.'.format(
                        get_function_name(),
                        status,
                        config_dev["handle_local"]["output_circuit"],
                    )
                ...
                logging.error(
                    '{}: "status" not found while running "switch loop"'.format(
                        get_function_name()
                    )
                )
        else:
            logging.error(
                "{}: Tried to toggle device  {} but failed with http return code '{}' .".format(
                    get_function_name(),
                    config_dev["handle_local"]["output_circuit"],
                    success,
                )
        ...
        logging.error(
            "{}: Unhandled exception in function config type: {}".format(
                get_function_name(), config_dev["handle_local"]["type"]
            )
        )
    logging.debug("{}: EOF.".format(get_function_name()))
...
def mqtt_ack(topic, message):
    ...
    logging.debug(
        '         {}: Starting function on topic "{}" with message "{}".'.format(
            get_function_name(), topic, message
        )
    )
    if topic.endswith("/set"):
        topic = topic[:-4]
        logging.debug(
            '         {}: Removed "set" from state topic, is now "{}" .'.format(
                get_function_name(), topic
            )
        )
    if topic.endswith("/brightness"):
        topic = topic[:-11]
        logging.debug(
            '         {}: Removed "/brightness" from state topic, is now "{}" .'.format(
                get_function_name(), topic
            )
        )
    ...
    if "mqtt_reply_message" in message:
        ...
        logging.debug(
            '         {}:Found "mqtt_reply_message" key in message "{}", changing reply message.'.format(
                get_function_name(), message
            )
        )
        ...
        logging.debug(
            '         {}:UNchanged return message, remains "{}" .'.format(
                get_function_name(), message
            )
        )
    ...
    return_message = json.dumps(
        message
    )  # we need this due to the fact that some MQTT message need a retun value of ON or OFF instead of original message
    mqttc.publish(
        topic, return_message, qos=0, retain=True
    )  # You need to confirm light status to leave it on in HASSIO
    logging.debug(
        '         {}: Returned topic is "{}" and message is "{}".'.format(
            get_function_name(), return_message, topic
        )
    )
    logging.debug("         {}: EOF.".format(get_function_name()))
...
def on_mqtt_connect(mqtt1, userdata, flags, rc):
    logging.info(
        "{}: MQTT Connected with result code {}.".format(get_function_name(), str(rc))
    )
    mqtt1.subscribe(
        unipiConfig["setTopic"]
...
def mqtt_online():  # function to bring MQTT devices online to broker
    for dd in unipiConfig["circuits"]:
        ...
        logging.info(
            '{}: MQTT "online" command to topic "{}" send.'.format(
                get_function_name(), mqtt_topic_online
            )
        )


def on_mqtt_subscribe(mqtt1, userdata, mid, granted_qos):
    logging.info(
        "{}: Subscribed with details: mqttc: {}, userdata: {}, mid: {}, granted_qos: {}.".format(
            get_function_name(), mqtt1, userdata, mid, granted_qos
        )
    )


def on_mqtt_disconnect(mqtt1, userdata, rc):
    logging.critical(
        "{}: MQTT DISConnected from MQTT broker with reason: {}.".format(
            get_function_name(), str(rc)
        )
    )  # Return Code (rc)- Indication of disconnect reason. 0 is normal all other values indicate abnormal disconnection
    if str(rc) == 0:
        mqtt1.unsubscribe(unipiConfig["setTopic"])
        mqtt_offline()
...
def mqtt_offline():  # function to bring MQTT devices offline to broker
    for dd in unipiConfig["circuits"]:
        ...
        logging.warning(
            '{}: MQTT "offline" command to topic "{}" send.'.format(
                get_function_name(), mqtt_topic_offline
            )
        )
    mqttc.disconnect()


def on_mqtt_unsubscribe(mqtt1, userdata, mid, granted_qos):
    logging.info(
        "{}: Unsubscribed with details: mqttc: {}, userdata: {}, mid: {}, granted_qos: {}.".format(
            get_function_name(), mqtt1, userdata, mid, granted_qos
        )
    )
    ...
    while True:
        try:
            ...
            ws = websocket.WebSocketApp(
                "ws://" + ws_server + "/ws",  # header=ws_header,
                on_open=on_ws_open,
...
def on_ws_open(ws):
    logging.error(
        "{}: WebSockets connection is starting in a separate thread!".format(
            get_function_name()
        )
    )
    firstrun()
...
def on_ws_message(ws, message):
    ws_sanity_check(
        message
    )  # This is starting the main message handling for UniPi originating messages


# print(ws)


def on_ws_close(ws: websocket, not_used, not_used2):
    logging.critical(
    ...
    if ws.isAlive():
        ws.join()
        logging.error(
            "{}: Joined websocket thread into main thread to cleanup thread.".format(
                get_function_name()
            )
        )
    else:
        logging.error(
            "{}: WebSockets thread was not foundrunning, in reconnect loop?".format(
                get_function_name()
            )
        )
...
def firstrun():
    for config_dev in unipiConfig["circuits"]:
        ...
        try:
            ...
            logging.info(
                "{}: Set status for dev: {}, circuit: {} to message and values: {}".format(
                    get_function_name(),
                    config_dev["dev"],
                    config_dev["circuit"],
                    message,
...
if __name__ == "__main__":
    ...
    urllib3_log = logging.getLogger(
        "urllib3"
    )  # ignoring informational logging from called modules (rest calls in this case) https://stackoverflow.com/questions/24344045/how-can-i-completely-remove-any-logging-from-requests-module-in-python
    ...
    t_mqtt = threading.Thread(
        target=mqttc.loop_forever
    )  # define a thread to run MQTT connection
